...
class ContinuousScatterPlot:
    def __init__(self, limits=(5, 5)):
        self.limits = limits
        self.window_size = (400, 400)  # Set the size of the plot window
        self.canvas = np.ones((self.window_size[0], self.window_size[1], 3), dtype=np.uint8) * 255
        self.multi = self.window_size[0] / limits[0]
        self._camera_position = (int(self.window_size[0]/2), int(self.window_size[0]/2)) 
        self._color = (255, 0, 0)
        self._thickness = 1
        self._font = 1
    def update_plot(self, new_x, new_y):
        ...
        cv2.circle(self.canvas, (int(scaled_x), int(scaled_y)), 10, (0, 0, 0), -1)
        cv2.circle(self.canvas, self._camera_position, 10, (0, 255, 0), -1)
        
        cv2.putText(self.canvas, 'Apriltag', (scaled_x,scaled_y),cv2.FONT_HERSHEY_SIMPLEX,self._font, \
            self._color, self._thickness, cv2.LINE_AA)
        cv2.putText(self.canvas, 'camera', self._camera_position,cv2.FONT_HERSHEY_SIMPLEX,self._font, \
            self._color, self._thickness, cv2.LINE_AA)
